...
fn version() {
  ...
  eprintln!("[RSVIM] Writing version into {output_path1:?}...");
...
fn snapshot() {
  ...
  let snapshot = {
    ...
    eprintln!("[RSVIM] Snapshot blob size is {snapshot_len} before compress...");
    ...
    eprintln!("[RSVIM] Compress snapshot with zstd-level={max_compress_level}...");
