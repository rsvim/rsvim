...
impl NormalStateful {
  ...
  fn cursor_move(&self, data_access: &StatefulDataAccess, op: Operation) -> StatefulValue {
    ...
    debug_assert!(matches!(cursor_parent_node, TreeNode::Window(_)));
    let buffer = match cursor_parent_node {
      TreeNode::Window(window) => window.buffer(),
      _ => unreachable!(),
    };
