...
mod test_static_import {
  ...
  util.echo(1);
    ...
    // After running
    {
      ...
      assert_eq!(
        Some("1".to_string()),
        contents.cmdline_message_history_mut().pop()
      );
    }
...
export function echoMain(value) {
  ...
  Rsvim.cmd.echo(`${isMain}:${value}`);
    ...
    // After running
    {
      ...
      let filename = contents.cmdline_message_history_mut().pop();
      ...
      let dirname = contents.cmdline_message_history_mut().pop();
      ...
      let is_main = contents.cmdline_message_history_mut().pop();
      assert!(is_main.is_some());
      let actual = is_main.unwrap();
      info!("main:{:?}", actual);
      assert_eq!(actual, "false:true");

      let resolved_module_path = contents.cmdline_message_history_mut().pop();
...
setTimeout(() => {
  ...
  Rsvim.rt.exit();
    ...
    // After running
    {
      ...
      let url2 = contents.cmdline_message_history_mut().pop();
      assert!(url2.is_some());
      let actual = url2.unwrap();
      info!("url2:{:?}", actual);
      assert!(actual.contains("TypeError: Module `null` not found"));

      let url3 = contents.cmdline_message_history_mut().pop();
      assert!(url3.is_some());
