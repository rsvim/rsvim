...
export interface RsvimCmd {
   ...
   * :::
   *
   * @param {string} name - The command name that is going to create. Only letters (`a-z` and `A-Z`), digits (`0-9`), underscore (`_`) and exclamation (`!`) are allowed in a command name. And it must begin with either a letter or underscore, digit is not allowed.
