...
export interface RsvimCmd {
   ...
   * @param {RsvimCmd.CommandAttributes} attributes - Attributes that control the command behavior, such as whether takes a `!` modifier (bang), how many arguments it takes (nargs), etc. This parameter can be omitted, it will use the default attributes, see {@link RsvimCmd.CommandAttributes}.
   * @param {RsvimCmd.CommandOptions} options - Options that control how the command is created, such as `force`, etc. This parameter can be omitted, it will use the default options, see {@link RsvimCmd.CommandOptions}.
