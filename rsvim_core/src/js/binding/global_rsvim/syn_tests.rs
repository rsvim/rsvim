...
async fn test_load1() -> IoResult<()> {
  ...
  try {
    ...
    await Rsvim.syn.loadTreeSitterParser({ grammarPath: "./tests_and_benchmarks/tree-sitter-python" });
    await Rsvim.syn.loadTreeSitterParser({ grammarPath: "./tests_and_benchmarks/tree-sitter-rust" });
  } catch (e) {
    Rsvim.cmd.echo(e);
  }
"#;
