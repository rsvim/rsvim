...
mod tests_nowrap {
  ...
  fn update1() {
    ...
    let viewport = {
      let buffer = lock!(buffer);
      let actual_shape = U16Rect::new((0, 0), (terminal_size.width(), terminal_size.height()));
      let viewport = Viewport::view(&win_opts, buffer.text(), &actual_shape, 4, 0);
      Viewport::to_arc(viewport)
    };
    let actual = make_canvas(terminal_size, win_opts, buffer.clone(), viewport.clone());
    assert_canvas(&actual, &expect);
...
mod tests_nowrap_startcol {
  ...
  fn update1() {
    ...
    let viewport = {
      let buffer = lock!(buffer);
      let actual_shape = U16Rect::new((0, 0), (terminal_size.width(), terminal_size.height()));
      let viewport = Viewport::view(&win_opts, buffer.text(), &actual_shape, 4, 5);
      Viewport::to_arc(viewport)
    };
    let actual = make_canvas(terminal_size, win_opts, buffer.clone(), viewport.clone());
    assert_canvas(&actual, &expect);
...
mod tests_wrap_nolinebreak {
  ...
  fn update1() {
    ...
    let viewport = {
      let buffer = lock!(buffer);
      let actual_shape = U16Rect::new((0, 0), (terminal_size.width(), terminal_size.height()));
      let viewport = Viewport::view(&win_opts, buffer.text(), &actual_shape, 3, 0);
      Viewport::to_arc(viewport)
    };
    let actual = make_canvas(terminal_size, win_opts, buffer.clone(), viewport);
    assert_canvas(&actual, &expect);
  ...
  fn update2() {
    ...
    let viewport = {
      ...
      let viewport = Viewport::view(&win_opts, buffer.text(), &actual_shape, 6, 0);
      Viewport::to_arc(viewport)
    };
    let actual = make_canvas(terminal_size, win_opts, buffer.clone(), viewport);
    assert_canvas(&actual, &expect);
...
mod tests_wrap_nolinebreak_startcol {
  ...
  fn update3() {
    ...
    let viewport = {
      let buffer = lock!(buffer);
      let actual_shape = geo_size_into_rect!(terminal_size, u16);
      let viewport = Viewport::view(&win_opts, buffer.text(), &actual_shape, 6, 4);
      Viewport::to_arc(viewport)
    };
    let actual = make_canvas(terminal_size, win_opts, buffer.clone(), viewport);
    assert_canvas(&actual, &expect);
...
mod tests_wrap_linebreak {
  ...
  fn update1() {
    ...
    let viewport = {
      let buffer = lock!(buffer);
      let actual_shape = U16Rect::new((0, 0), (terminal_size.width(), terminal_size.height()));
      let viewport = Viewport::view(&win_opts, buffer.text(), &actual_shape, 2, 0);
      Viewport::to_arc(viewport)
    };
    let actual = make_canvas(terminal_size, win_opts, buffer.clone(), viewport.clone());
    assert_canvas(&actual, &expect);
