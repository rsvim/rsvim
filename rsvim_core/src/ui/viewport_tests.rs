...
mod tests_search_anchor_downward_wrap_nolinebreak {
  ...
  fn new3() {
    ...
    // Search-2
    {
      ...
      let actual =
        search_down_viewport(&mut tree, window_id, buf.clone(), 2, 37, 2, 24);
...
mod tests_search_anchor_upward_wrap_nolinebreak {
  ...
  fn new8() {
    ...
    // Search-6
    {
      let expect = vec!["\ta\t", "quite\tsimp", "le\tand", "\tsmall", "\ttest"];

      let actual =
        search_up_viewport(&mut tree, window_id, buf.clone(), 1, 32, 1, 14);
