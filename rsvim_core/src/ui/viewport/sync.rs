...
fn _end_char_and_prefills(
  ...
  if c_width > end_width {
    ...
    let next_to_last_visible_char =
      text.last_char_on_line_exclude_eol(l).unwrap_or(0_usize) + 1;
    ...
    match text.char_after(current_line, start_column) {
      Some(mut start_char) => {
        ...
        while current_row < window_height {
          ...
          if end_char
            > text
              .last_char_on_line_exclude_eol(current_line)
              .unwrap_or(0_usize)
          {
            break;
          }
...
mod detail {
  ...
  pub fn cursor_width_to_left_no_eol(
    ...
    if target_is_eol {
      target_cursor_width =
        match text.last_char_on_line_exclude_eol(target_cursor_line) {
...
mod wrap_detail {
  ...
  fn to_left_1(
    ...
    let last_char = text
      .last_char_on_line_include_eol(target_cursor_line) // Also consider eol.
      .unwrap_or(0_usize);
...
pub fn search_anchor_downward(
  ...
  let target_cursor_char = std::cmp::min(
    target_cursor_char,
    text
      .last_char_on_line_include_eol(target_cursor_line)
      .unwrap_or(0_usize),
  );
