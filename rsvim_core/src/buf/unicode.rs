...
pub fn char_width(opt: &TextOptions, c: char) -> usize {
...
pub fn char_symbol(opt: &TextOptions, c: char) -> (CompactString, usize) {
...
pub fn str_width(opt: &TextOptions, s: &str) -> usize {
  s.chars().map(|c| char_width(opt, c)).sum()
}

/// Get the printable cell symbols and the display width for a unicode `str`.
pub fn str_symbols(opt: &TextOptions, s: &str) -> (CompactString, usize) {
