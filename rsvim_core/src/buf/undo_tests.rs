...
fn assert_insert(
  ...
  assert!(matches!(actual, ChangeOp2::Insert(_)));
  match actual {
    ChangeOp2::Insert(insert) => {
...
fn assert_delete(
  ...
  assert!(undo_manager.current().operations().len() > op_idx);
  let actual = &undo_manager.current().operations()[op_idx];
  assert!(matches!(actual, ChangeOp2::Delete(_)));
  match actual {
    ChangeOp2::Delete(delete) => {
...
fn insert1() {
  ...
  for (i, c) in payload.chars().enumerate() {
    undo_manager.save(ChangeOp2::Insert(Insert2 {
      char_idx: i,
...
fn delete1() {
  ...
  undo_manager.save(ChangeOp2::Delete(Delete2 {
    char_idx: 12,
