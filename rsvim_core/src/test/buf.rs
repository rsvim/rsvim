...
use crate::buf::{Buffer, BufferArc, BuffersManager, BuffersManagerArc, LocalOptions};
use crate::rlock;
...
pub fn make_buffer_from_file(opts: LocalOptions, filename: String) -> BufferArc {
  let rp = make_rope_from_file(filename);
  let bf = Buffer::_new(rp, opts, None, None, None, None);
  Buffer::to_arc(bf)
}

pub fn make_buffer_from_lines(opts: LocalOptions, lines: Vec<&str>) -> BufferArc {
  let rp = make_rope_from_lines(lines);
  let buf = Buffer::_new(rp, opts, None, None, None, None);
  Buffer::to_arc(buf)
}

pub fn make_empty_buffer(opts: LocalOptions) -> BufferArc {
  let buf = Buffer::_new_empty(opts);
  Buffer::to_arc(buf)
}

pub fn make_buffer_from_rope(opts: LocalOptions, rp: Rope) -> BufferArc {
  let buf = Buffer::_new(rp, opts, None, None, None, None);
  Buffer::to_arc(buf)
}

pub fn make_buffers_manager(opts: LocalOptions, bufs: Vec<BufferArc>) -> BuffersManagerArc {
