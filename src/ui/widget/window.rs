...
use crate::ui::tree::{GlobalOptions, Tree, TreeWk};
...
///
/// For the text contents inside a window, here introduce several terms and concepts:
///
...
impl Window {
  pub fn new(shape: IRect, buffer: BufferWk, tree: TreeWk) -> Self {
    let options = tree
      .upgrade()
      .unwrap()
    ...
    match base.node_mut(&window_content_id).unwrap() {
      WindowNode::WindowContent(ref mut content) => content.sync_cells_size(),
      _ => unreachable!("Failed to get window content node"),
    }
...
impl Widgetable for Window {
  fn draw(&mut self, canvas: &mut Canvas) {
    // Resize window content, because when create window content, it doesn't know itself's actual
    // shape.

    unsafe {
      // Fix multiple mutable references on self.
      let mut raw_self = NonNull::new(self as *mut Window).unwrap();
      match raw_self.as_mut().base.node_mut(&self.content_id).unwrap() {
...
impl Window {
  ...
  pub fn _draw_from_top(
    ...
    match (self.wrap(), self.line_break()) {
      ...
      (true, true) => {
        self._draw_from_top_for_wrap_linebreak(content, start_line, start_column, end_column)
      }
    }
  }
              ...
              for chunk in line.chunks() {
                ...
                for ch in chunk.chars() {
                  ...
                  if ch != '\n' {
                    ...
                    // );
                    content.set_cell(cell_upos, cell);
                  }
                  col += 1;
                }
              }
  ...
  /// - [`line_break`](WindowOptions::line_break) will not be used since 'wrap' is `false`.
  ...
  /// Implement the [`_draw_from_top`] with below window options:
  /// - [`warp`](WindowOptions::wrap) is `true`.
  /// - [`line_break`](WindowOptions::line_break) is `true`.
...
mod tests {
  ...
  fn _draw_from_top_for_nowrap1() {
    ...
    let tree = Tree::to_arc(Tree::new(terminal_size));

    {
      // wrap=false
      let window_options = WindowLocalOptions::builder().wrap(false).build();
      tree.write().global_options_mut().window_local_options = window_options;
    }

    let window_shape = IRect::new((0, 0), (10, 10));
    let mut window = Window::new(window_shape, Arc::downgrade(&buffer), Arc::downgrade(&tree));
    unsafe {
      let mut raw_window = NonNull::new(&mut window as *mut Window).unwrap();
      match raw_window
        .as_mut()
        .base
        .node_mut(&window.content_id)
        .unwrap()
      {
  ...
  fn _draw_from_top_for_nowrap2() {
    ...
    unsafe {
      ...
      match raw_window
        .as_mut()
        .base
        .node_mut(&raw_window.as_mut().content_id)
        .unwrap()
      {
        WindowNode::WindowContent(ref mut content) => {
          raw_window
    ...
    unsafe {
        ...
        .unwrap()
      {
        WindowNode::WindowContent(ref mut content) => {
          raw_window
            .as_mut()
            ._draw_from_top_for_wrap_nolinebreak(content, 1, 0, 0);
          let actual = content
            .frame()
            .raw_symbols_with_placeholder(" ".to_compact_string())
            .iter()
    ...
    unsafe {
        ...
        .unwrap()
      {
        WindowNode::WindowContent(ref mut content) => {
          raw_window
            .as_mut()
            ._draw_from_top_for_wrap_nolinebreak(content, 0, 0, 0);
          let actual = content
            .frame()
            .raw_symbols_with_placeholder(" ".to_compact_string())
            .iter()
    ...
    unsafe {
        ...
        .unwrap()
      {
        WindowNode::WindowContent(ref mut content) => {
          raw_window
            .as_mut()
            ._draw_from_top_for_wrap_linebreak(content, 0, 0, 10);
          let actual = content
            .frame()
            .raw_symbols_with_placeholder(" ".to_compact_string())
            .iter()
    ...
    unsafe {
        ...
        .unwrap()
      {
        WindowNode::WindowContent(ref mut content) => {
          raw_window
            .as_mut()
            ._draw_from_top_for_wrap_linebreak(content, 1, 0, 0);
          let actual = content
            .frame()
            .raw_symbols_with_placeholder(" ".to_compact_string())
            .iter()
    ...
    unsafe {
        ...
        .unwrap()
      {
        WindowNode::WindowContent(ref mut content) => {
          raw_window
            .as_mut()
            ._draw_from_top_for_wrap_linebreak(content, 0, 0, 0);
          let actual = content
            .frame()
            .raw_symbols_with_placeholder(" ".to_compact_string())
            .iter()
